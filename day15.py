...
class Grid(dict):
    def __init__(self, lines: List[str], power: int = 3):
        super().__init__()
    ...
    def play(self, elf_death: bool = False):
        rounds = 0
        while True:
    ...
    def move(self, unit: Unit, elf_death: bool = False):
        ...
        in_range = set(pt for target in targets for pt in target.position.nb4
    ...
    def find_move(self, position: Unit, targets: List[Unit]) -> Point:
        ...
        try:
            min_dist, closest = min((dist, pos) for pos, (dist, parent) in
             meta.items() if pos in targets)
        except ValueError:
            return
